  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6  gap-3 relative">
        {cart && cart?.length > 0 ? (
          <>
            <div className="flex flex-col w-full items-start justify-start gap-3 h-[50%] overflow-y-scroll scrollbar-none px-4">
              {cart &&
                cart?.length > 0 &&
                cart?.map((item, i) => (
          ...
          <>
            <h1 className="text-3xl mx-3 my-3 text-primary font-bold">Empty Cart</h1>
          </>
        )}
      </div>
    </motion.div>
